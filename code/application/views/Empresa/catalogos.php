			...
			<div class="wrapper wrapper-content animated fadeInRight" style="padding: 20px 10px 0px !important;"><!-- wrapper-content -->
				<a href="#" id="aDownload"></a> <!-- href es primero! no cambiar posición -->
                <div class="row">
	                <div class="col-md-12">
			...
			<div class="wrapper wrapper-content animated fadeInRight" style="padding-bottom:80px;"><!-- wrapper-content -->
				...
				<?php
					if(isset($result) && $result != false){
						...
						$sizePdf = round(($v["size"]/(1000*1000))*100)/100;
						$sizeRar = round(($v["size_rar"]/(1000*1000))*100)/100;
						...
						echo '				<span class="product-price"> '.$sizePdf.'MB (PDF) / '.$sizeRar.'MB (RAR)</span>'; //agregar tamaño de size pa pdf y rar
						...
						if($v["has_pdf"] == 1) 
							echo '														<li><a href="javascript:downloadFile(\''.$uridownload_pdf.'\');">Descargar en .PDF</a></li>';
						if($v["has_rar"] == 1)
							echo '														<li><a href="javascript:downloadFile(\''.$uridownload_rar.'\');">Descargar en .RAR</a></li>';
						echo '												</ul>';
						echo '										</div>';
						echo '			</div>';
						echo '		</div>';
	...
	function catalogo_familia(value, categoria)
        {
            ...
            $("#"+categoria).find("option[value='-1']").prop('selected',true);
				...
				function downloadFile(uri){//mode = 1 add , 0 upd
						...
						var idShow = "#info_callback";
							...
							$.ajax({
									...
									success: function(dataResponse, status, request)          //on recieve of reply
									{
									...
									//--------------------------------------------------------------------
											...
											var anchor = document.getElementById("aDownload");
											//$(anchor).attr("href",  data.path);
											//$(anchor).attr("download",  "003-quimicosdeaseo.pdf");
											$(anchor).prop({"href":  dataResponse.path,
																			"download":  dataResponse.filename});
